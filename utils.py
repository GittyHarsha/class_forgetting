...
def get_dataset(args):
    if "fmnist" in args.dataset:
        ...
        # Load the GTSRB train and test datasets
        dataset1 = datasets.GTSRB(root=args.data_path, split='train', download=True, transform=transform)
        dataset2 = datasets.GTSRB(root=args.data_path, split='test', download=True, transform=transform)
        
        # Set number of classes and label names
        args.num_classes = 43  # GTSRB has 43 classes
        args.class_label_names = [i for i in range(43)]  # Label names as integers 0-42
    else:
        raise ValueError
